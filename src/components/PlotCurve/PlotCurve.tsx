...
import {DisplayParametersFroms} from '../Steps/Step/DisplayParametersForms';
...
const PlotCurve: React.FC<PlotCurveProps> = (props) => {
  ...
  const[begins,setBegins] = useState(true);
  ...
  useEffect(() => {
    ...
    if(displayInitCurves){
        ...
        for(let i=0; i<props.curves.length; i++){
          if(props.curves[i].markerId){
            ...
            if(begins){
              ...
              const point2 = { type: 'scatter', mode: 'markers', name: props.curves[i].name, opacity: opacity, marker: { color: 'pink', symbol: ['x'], size: 10 }, x: [x_marker2], y: [y_marker2] };
            data_.push(point2);
            }else{
      ...
      for(let i=0; i<props.curves.length; i++){
        ...
        const newLine :any ={
          name:props.curves[i].name,
          strainMaxValue:Math.max(...props.curves[i].x0),
          stressMaxLimit:Math.max(...props.curves[i].y0)
        }
        FinalArr_.push(newLine);
      }
      let averageMaxStrainValue:number=0;
      let finalStrainValue:number=0;
      let averageMaxStressValue:number=0;
      let finalStressValue:number=0;
      for(let i=0; i<FinalArr_.length; i++){
        averageMaxStrainValue+=FinalArr_[i].strainMaxValue; 
        averageMaxStressValue+=FinalArr_[i].stressMaxLimit;
      }
      finalStrainValue = averageMaxStrainValue/FinalArr_.length;
      finalStressValue = averageMaxStressValue/FinalArr_.length;
        console.log("finalStrainValue",finalStrainValue);
        console.log("finalStressValue",finalStressValue);
        if(finalStrainValue!=-Infinity&&finalStressValue!=-Infinity){
          setFinalStrainValue(finalStrainValue);
          setFinalStressValue(finalStressValue);
          setDisplayParam(true);
          // callDisplayParams();
        }

        // else{
        //   setDisplayParnam(false);
        // }
        
         
      //  navigate('../Steps/Step/DisplayParametersForms',{ state: { maxStrainValue:finalStrainValue,maxStressValue: finalStressValue}})
      if(props.showMarkers){
        for(let i=0; i<props.curves.length; i++){
          if(props.curves[i].markerId){
            ...
            // var point;
            if(begins){
            const point = { type: 'scatter', mode: 'markers', name: props.curves[i].name, opacity: opacity, marker: { color: 'green', symbol: ['x'], size: 10 }, x: [x_marker], y: [y_marker] };
            // data_.push(point);
            }else{
  ...
  const callDisplayParams =()=>{
    return(
      <>
    {displayParam&&<DisplayParametersFroms 
      finalStrainValue={finalStrainValue} 
      finalStressValue={finalStressValue}></DisplayParametersFroms>}
    
    </>
    )
  }
  const AddPoint = (data_point: any) =>{
    if(props.action==='Cleaning_begins'){
      setBegins(true);
    }else{
      setBegins(false);
    ...
    if(props.mode.plotMode===PlotMode.Consolidation&&props.mode.consolidationAlgo==='failure'){
      ...
      if(c){ // replace by new value
        ...
        if(begins){
          ...
          data_up.push(point);
        }else{ const point = { type: 'scatter', mode: 'markers', name: curve_name,  marker: { color: 'black', symbol: ['x'], size: 10 }, x: [x], y: [y] };
        data_up.push(point);
        }
      }
